...
class Canvas {
  ...
  createPreloader() {
    ...
    this.items.forEach(src => {
      ...
      image.src = src;
      image.onload = () => {
        if (++this.loaded === this.items.length && typeof document !== 'undefined') {
  ...
  onWheel(e: WheelEvent) {
    // Non-blocking wheel update: adds to target movement without preventDefault
    this.scroll.target += e.deltaY * 0.006;
  }

  onWindowScroll() {
    if (typeof window === 'undefined') return;
    const currentY = window.scrollY;
    const diff = currentY - this.lastWindowScrollY;
    this.lastWindowScrollY = currentY;
    this.scroll.target += diff * 0.015;
  }
  ...
  addEventListeners() {
    ...
    // Attach mouse & touch drag events strictly to the container so regular page scrolling is NOT blocked
    if (this.container) {
...
export default function FlyingPosters({
  ...
  return (
    <div
      ref={containerRef}
      className={`w-full h-full overflow-hidden relative z-2 select-none ${className}`}
